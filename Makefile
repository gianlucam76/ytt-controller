...
create-cluster: $(KIND) $(KUBECTL) $(ENVSUBST) ## Create a new kind cluster designed for development
	...
	@echo "prepare configMap with ytt files"
	$(KUBECTL) create configmap ytt --from-file=ytt.tar.gz=test/ytt.tar.gz

	@echo "prepare configMap with ytt overlays files"
	$(KUBECTL) create configmap ytt-overlays --from-file=ytt.tar.gz=test/ytt_overlays.tar.gz
...
deploy-ytt-controller: $(KUSTOMIZE)
	# Load ytt controller image into cluster
	@echo 'Load ytt-controller image into cluster'
	$(MAKE) load-image

	# Install ytt-controller
	@echo 'Install ytt-controller'
	cd config/manager && $(KUSTOMIZE) edit set image controller=${IMG}
	$(KUSTOMIZE) build config/default | $(ENVSUBST) | $(KUBECTL) apply -f-

	@echo "Waiting for ytt-controller to be available..."
	$(KUBECTL) wait --for=condition=Available deployment/ytt-controller -n ytt-system  --timeout=$(TIMEOUT)
